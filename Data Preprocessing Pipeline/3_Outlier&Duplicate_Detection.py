...
class DataAuditor:
    ...
    def detect_and_drop_duplicates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        【最適合處理的資料情境】
        常用於網路爬蟲重複抓取、系統伺服器重疊寫入、
        或是多個資料流管道合併時產生的數據。

        【統計原理與直覺本質】
        - 避免權重被放大：重複的觀測值會被人為地放大權重，導致平均數偏移、變異數被低估，
          模型甚至會去死記硬背重複的樣本（導致 Overfitting）。
          剔除重複值，還原數據集真實隨機抽樣結構。
    ...
    def detect_outliers_zscore(
        ...
        【最適合處理的資料情境】
        適用於已知或高度懷疑該數據分佈符合常態分佈的欄位

        【統計原理與直覺本質】
        - 常態分佈的邊界守護者：大自然與多數人類經濟行為在大量隨機抽樣下，皆會收斂成常態分佈。
          如果 |Z| > 3，意味著這個事件在常態世界中發生的機率小於 0.3%，屬於超級罕見的特例。
          在計量經濟學中，這通常代表著資料錄入錯誤或是發生極端事件。
    ...
    def detect_outliers_iqr(
        ...
        【最適合處理的資料情境】
        適合處理非正常分佈、嚴重偏態或含有大量極端值的情境，

        【統計原理與直覺本質】
        - 四分位距（IQR = Q3 - Q1）則是利用資料的排位（第 25% 與第 75% 的切點）。
          無論極端值有多大，都不會影響 Q1 與 Q3 的位置。
    ...
    def detect_outliers_isolation_forest(
        ...
        【最適合處理的資料情境】
        適用於高維度欄位且欄位間具備複雜的交叉複合異常情境。

        【統計原理與直覺本質】
        - 隔離森林採取隨機切分。
          如果我們對空間中的資料隨機畫切刀（隨割二元樹），那些聚集成群的正常資料，需要切非常多刀才能被單獨分離出來；
          相反地，那些特徵怪異、不合群的「異常觀測值」，因為在空間中結構孤立，只要簡短的幾刀就會被切分出來。
